...
describe('Image API', () => {
    ...
    describe('POST /api/users/:userId/image', () => {
        ...
        it('should create post image', async () => {
            ...
            const image = process.cwd() + '/test/profile.png'

            const result = await supertest(app).post(`/api/users/${user.id}/image`)
        ...
        it('should reject if value path :userId not equal id in jwt', async () => {
            ...
            const result = await supertest(app).post(`/api/users/salah/image`)
              .set('AUTHORIZATION', `Bearer ${token.body.data.token}`)
              .attach('image', image)
              .field('title', 'imute')
              .field('description', 'ini sangat menarik');
    ...
    describe('GET /api/users/:userId/image', () => {
        ...
        it('should get image with userId', async () => {
            ...
            const result = await supertest(app).get(`/api/users/${user.id}/image`)
                .set('AUTHORIZATION', `Bearer ${token.body.data.token}`);
    ...
    describe('GET /api/users/:userId/image/:imageId', () => {
        ...
        it('should get detail image', async () => {
            ...
            const result = await supertest(app).get(`/api/users/${user.id}/image/${image.id}`)
                .set('AUTHORIZATION', `Bearer ${token.body.data.token}`);
        ...
        it('should reject if image not found', async () => {
            ...
            const result = await supertest(app).get(`/api/users/${user.id}/image/salah`)
                .set('AUTHORIZATION', `Bearer ${token.body.data.token}`);
